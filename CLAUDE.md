...
1. **`src/extension/`** — DuckDB integration, with TWO independent targets (see the comment atop `src/extension/CMakeLists.txt`):
   ...
   - Note: `GPUDB_FORCE_BACKEND` is NOT currently honored — `test/sql/gpu_force_backend.test` only pins that setting it doesn't crash. The env overrides that ARE honored (all read via `getenv` in `src/`, plus one in `python/`; none of them changes a query's answer, only which path runs, how much host memory it may use, or what it prints):
     ...
     - CUDA backend: `GPUDB_CUDA_EXACT` (`0` disables the CUDA exact path — `exact_supported()`, and with it `global_supported()`, `join_supported()` and the placement of exact-upload columns on the device. On by default: the path is complete and the wrapper suite and `scripts/tpch_coverage.py` have both been run against it. The switch exists because a resident column is single-homed, so turning it off is the only way to put a set back behind the CPU reference without a rebuild).
     - Metal backend, path selection and sweeps: `GPUDB_METAL_GROUPBY_PATH` (v0.6 GROUP BY: slot-lock vs radix-sort), `GPUDB_METAL_HASHJOIN_PATH` (hash-join algorithm), `GPUDB_METAL_GROUPBY_EXACT_PATH` (`direct|sort|auto` for the exact GROUP BY), `GPUDB_METAL_DIRECT_KERNEL` (`thread|slab|auto` accumulator shape), `GPUDB_METAL_DIRECT_MIN_WORK` / `GPUDB_METAL_DIRECT_MIN_GROUPS` / `GPUDB_METAL_DIRECT_MAX_GROUPS` / `GPUDB_METAL_DIRECT_THREAD_SLOTS` (the measured admission thresholds for the direct path), `GPUDB_METAL_DIRECT_DISABLE_PSO` (make chosen direct pipelines refuse to build, to test the fallback), `GPUDB_METAL_MASK_COMPACT_BELOW` (selectivity at which a WHERE compacts before reducing), `GPUDB_METAL_MASK_PATH` (`fused|legacy|permeval|auto` for the sort path's WHERE stage; read per call so a measurement can interleave the shapes), `GPUDB_METAL_MASK_DISABLE_PSO` (make the fused mask pipelines refuse to build, to test the fallback), `GPUDB_METAL_HOST_FILTER_BELOW` (group count below which top-k runs on the host), `GPUDB_METAL_PREWARM` (`0` = prepare() builds the sort cache only), `GPUDB_METAL_TRACE_EXACT` (per-stage GPU times on stderr), `GPUDB_METAL_UPLOAD_REFUSE_MB` (make the backend refuse any exact upload above this many MiB — read once at load; the only way to reach the device-refusal paths on a machine with memory to spare, used by `scripts/budget_gate.py --refuse-mb`).
     - Python wrapper: `GPUDB_MEMORY_BUDGET_MB` (overrides the default device-memory budget); `GPUDB_UPLOAD_QUIET_MS` (§5.5 back-off: the idle stretch a background-upload step an interrupt cannot stop — `gpu_upload_finish`, the sort cache — asks for on a machine whose cores are contended, default 400 ms; `0` turns the back-off off), `GPUDB_UPLOAD_QUIET_MAX_S` (how long it waits for that window before taking it anyway, default 20 s — the bound on how long readiness can be delayed per step), `GPUDB_RESIDENCY_TRACE` (one stderr line per segment attempt and per quiet wait: how long the session waited for its window, how long the scan ran, how many cores it got, whether it landed).
