...
| `gpu_upload_rows_exact(name, k BIGINT\|VARCHAR, v BIGINT, BIGINT[] preds, DOUBLE[] preds[, VARCHAR[] preds])` | `BIGINT` rows | v0.7 exact set with predicate lanes; a VARCHAR key is the wrapper's tuple text stored as `hash64()` with a dictionary; VARCHAR predicate lanes hold `hash64()` of the text (collisions detected) |
| `gpu_resident_dictionary(name, n)` | `(id BIGINT, c0 .. c<n-1> VARCHAR)` | the key tuples of a VARCHAR-keyed set split into their `n` components (`N` → NULL); joined on `r.key = d.id` |
| `gpu_resident_dict_component(name, key, i)` | `VARCHAR` | v0.7 §4.15: component `i` (0–7) of ONE key of a string-keyed set, NULL for a NULL component / NULL key / unknown key. The per-key form of `gpu_resident_dictionary`, emitted after a device HAVING / top-k where only a few keys survive |
| `gpu_note_rows(name, n)` | `BOOLEAN` | v0.7 §4.13: a column-less sentinel set that remembers `n`, a base table's row count when a set built from a JOIN over it was uploaded; `gpu_assert_rows(name, count(*))` over the table is then that table's staleness guard. Holds no device memory |
| `gpu_groupby_exact_multi(name, program, payloads, filter)` | `key BIGINT, count_star BIGINT`, then per payload `p`: `sum<p> HUGEINT, count<p> BIGINT, min<p> BIGINT, max<p> BIGINT, avg<p> DOUBLE` | v0.7 §4.9: the exact GROUP BY over several payload lanes of one set. `program` = WHERE program (`''` none); `payloads` = `'v, i0, ...'` (lane `v` or a BIGINT predicate lane, 1–8); `filter` = `''` \| `'having <p> <agg> <cmp> <threshold>'` \| `'topk <p> <agg> <k> <asc\|desc>'` with `<p>` an index into `payloads`. Projection pushdown skips unread payloads. On Metal the mask and the grouping are computed once for all payloads |
| `gpu_join_materialize(out, probe, probe_lane, build, build_lane, lanes)` | `BIGINT` joined rows | v0.7 §4.8: inner equi-join of two exact sets on the device, published as the exact set `out`. `build_lane` must be unique among its non-NULL cells (else `build key not unique`). `lanes` lists `p.<lane>` / `b.<lane>` (`k v i<n> f<n> s<n>`): new key, new payload (BIGINT), then predicate lanes — BIGINT, DOUBLE, VARCHAR in that order; string lanes keep their dictionaries. `out` is stale once a source set is stale, dropped or replaced |
