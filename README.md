...
- **🔗 Whole multi-table queries** — joins, `EXISTS`/`IN` subqueries, CTEs, views, derived tables, `HAVING`, `ORDER BY … LIMIT`, `count(DISTINCT)`, `DECIMAL`/`DATE`/`VARCHAR` keys: TPC-H SF10 is **19 of 22 on the device, 0 rows differing, 1.3–52.9×**. → [what runs where](#what-runs-on-the-gpu-and-what-stays-on-duckdb)
- **📊 Dashboards & monitoring** — the same aggregates every few seconds; a resident `SUM` turns a 99 ms scan into **10 ms** at 600M rows. → [Numbers](#numbers--measured-not-promised)
...
- **📈 High-cardinality GROUP BY / top-k** — `HAVING` and `ORDER BY … LIMIT k` run on the device, so only the survivors come back: **5.4–6.1× (Metal) / 13–24× (CUDA)** on Q18's inner query. → [BENCHMARK.md](BENCHMARK.md)
- **💰 DECIMAL / financial data** — scaled integers, 128-bit sums, bit-exact against native, and no per-scan cast: **9.9× (Metal) / 25× (CUDA)**. → [BENCHMARK.md](BENCHMARK.md)
- **🎯 Membership at scale** — `EXISTS`/`IN` in a `WHERE` becomes a predicate lane on the device: **22× (Metal) / ~376× (CUDA)** at SF10. → [BENCHMARK.md](BENCHMARK.md)
- **🧰 From any DuckDB client, by name** — the `gpu_*` functions, from the stock CLI or any binding; fused join-aggregates at **11.7× (Metal) / 27–37× (CUDA)** at SF50. → [Three ways in](#three-ways-in)
