...
- **📈 High-cardinality GROUP BY / top-k** — "quantity per order", "spend per customer", "top 10 by amount" over tens of millions of keys, re-asked as the data is explored. `gpu_groupby_sum_resident` returns `(key, sum, count)` rows (`gpu_groupby_count_resident` returns `(key, count)`) from a cached device sort — `WHERE` / `ORDER BY` / `LIMIT` compose on top natively: **5.0–5.6× (Metal)** on TPC-H Q18's inner query at SF10–SF50 and 3.4–4.7× on the plain 15M–75M-group `GROUP BY`; on CUDA **2.7× end-to-end (~45× on-device)** for Q18's inner query and 1.8× (~30×) for the plain GROUP BY, bounded by copying the groups back over PCIe; `gpu_topk_resident` is ~0 ms after the first call. Low-cardinality GROUP BY stays a native win on Metal (measured, kept in the table).
