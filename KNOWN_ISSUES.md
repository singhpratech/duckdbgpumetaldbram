...
| Behavior | Reason |
|---|---|
| `gpu_groupby_sum_resident_f64_having` / `_f64_topk` filter on the host, not the device | `DOUBLE` sums are finished on the host on Metal (no doubles in MSL) and the filter runs there through the reference implementation; the saving is only the row streaming (measured 1.4–2.6× vs native, against 5–8× for the BIGINT forms). CUDA filters f64 on the device. |
| `_topk` tie order is unspecified | When several groups share the k-th aggregate, which of them are returned — and in what order among equals — is backend-defined (Metal/CPU: key ascending; CUDA: radix-sort order). Add a tiebreaker in SQL if it matters; cross-backend checks compare multisets. |
...
| On CUDA, a device fault (e.g. an illegal address) raised *inside* a Thrust call aborts the process instead of surfacing as a SQL error | The fault is sticky on the CUDA context and Thrust's own temporary-buffer destructor then throws on `cudaFree`, which reaches `std::terminate` before gpudb's error mapping runs. Allocation failures, launch failures and every non-fault error are mapped to a clean `runtime_error`. No correct result is possible after such a fault anyway; replacing Thrust temporaries with explicit CUB temp storage would turn the abort into an error (v0.7 candidate). Not observed on any tested input. |
